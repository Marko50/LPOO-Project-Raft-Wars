...
public class GameStageController implements ContactListener {
    ...
    public void shootPlayerAmmo(int x, int y){
        if(GameStage.getInstance().getPlayerTurn() == 1){
            for(int i = 0; i < this.getBodiesPlayer1().size(); i++){
                if(GameStage.getInstance().getHeroesPlayer1().get(i).isSelected())
                {
                    GameStage.getInstance().chooseSelected(2);
                    this.getBodiesPlayer1().get(i).shootAmmo(x,y);
                    GameStage.getInstance().getHeroesPlayer1().get(i).getAmmo().setBeingUsed(true);
                    break;
                }
            }
        }
            ...
            for(int i = 0; i < this.getBodiesPlayer2().size(); i++){
                if(GameStage.getInstance().getHeroesPlayer2().get(i).isSelected())
                {
                    GameStage.getInstance().chooseSelected(1);
                    this.getBodiesPlayer2().get(i).shootAmmo(-x,-y);
                    GameStage.getInstance().getHeroesPlayer2().get(i).getAmmo().setBeingUsed(true);
                    break;
                }
            }
        }
    ...
    public void setBodiesPlayer2(ArrayList<CharacterBody> bodysPlayer2) {this.bodiesPlayer2 = bodysPlayer2;}
